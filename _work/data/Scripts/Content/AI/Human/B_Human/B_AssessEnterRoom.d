...
func int B_AssessEnterRoom ()
{
	...
	// Leohnard ist es egal, ob der Held in fremde Häuser geht

	if (Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Mod_7419_OUT_Leonhard_REL))
	{
		return FALSE;
	};
	...
	{
		if (Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Mod_799_BAU_Thekla_NW))
		{
			return FALSE;
		};
	};
	...
	var C_Item NovRuessi;
	NovRuessi = Npc_GetEquippedArmor(hero);

	if (portalguild == GIL_VLK)
	&& (Hlp_IsItem(NovRuessi, ITAR_FAKE_NOV_L) == TRUE)
	{
		return FALSE;
	};
