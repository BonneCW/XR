...
//###	KDW	###
	...
	if (Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Mod_7652_OUT_Fremder_REL))
	{
		...
		|| (spellType == SPL_ICERAIN)
		|| (spellType == SPL_ICECUBE)
		|| (spellType == SPL_ICELANCE)
		|| (spellType == SPL_INSTANTICEBALL)
	...
	{
		if (spellType == SPL_ArgezVision)
		{
			return COLL_DONOTHING|COLL_DONTKILL;
		};
	};
	...
	|| (self.aivar[AIV_MM_REAL_ID] == ID_SUMMONED_FIREGOLEM)
	{
	...
	if (self.aivar[AIV_MM_REAL_ID] == ID_FIRESHADOWBEAST)
	{
		if (spellType == SPL_ICEBOLT)
		|| (spellType == SPL_GEYSER)
		|| (spellType == SPL_ICERAIN)
		...
		|| (spellType == SPL_WATERFIST)
		{
			return COLL_APPLYDOUBLEDAMAGE; // Doppelter Schaden bei Wasser und Eis
		}
		...
		|| (spellType == SPL_PYROKINESIS_G1)
		{
			return COLL_DONOTHING; // Immun gegen Feuer
		}
		else
		{
			return COLL_DOEVERYTHING; // Und sonst normaler Schaden
		};
	};
	...
	if (self.aivar[AIV_MM_REAL_ID] == ID_ALRAUNE)
	|| (self.aivar[AIV_MM_REAL_ID] == ID_APFELBAUM)
	{
		if (spellType == SPL_FIREBOLT)
		|| (spellType == SPL_CHARGEFIREBALL)
		|| (spellType == SPL_FIRERAIN)
		...
		|| (spellType == SPL_PYROKINESIS_G1)
		{
			return COLL_APPLYDOUBLEDAMAGE; // Empfindlich gegen Feuer
		}
		else
		{
			return COLL_DOEVERYTHING; // Und sonst normaler Schaden
...
//----- Feuer -----	
	...
	|| (spellType  	== SPL_InstantFireball)	
	|| (spellType  	== SPL_GolemChargeFireball)	
	|| (spellType  	== SPL_Firerain)		
	...
	|| (spellType 	== SPL_Deathball)
	{
		...
		if (Npc_IsInState(self, ZS_MagicFreeze))
		{
			Npc_SetStateTime(self, 100);
...
// ---- Untote vernichten ----
	...
	|| (spellType == SPL_HolyWave)
	|| (spellType == SPL_HolyRain)
	{
		if (C_NpcIsUndead(self))
...
//----- Paladin Sprüche -----	
	...
	{
		if (C_NpcIsEvil(self))
		|| (Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Mod_7174_DEM_Randolph_NW))
		{
