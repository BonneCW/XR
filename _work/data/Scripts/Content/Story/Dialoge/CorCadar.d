...
{
	AI_Output(self, hero, "Info_Mod_Cadar_Hi_16_00"); //Ah, ein neues Gesicht in unserem Sumpf. Sei mir willkommen.
	AI_Output(self, hero, "Info_Mod_Cadar_Hi_16_01"); //Ich bin Cor Cadar, geistiger Anführer unserer Gemeinschaft.
};
...
{
	AI_Output(self, hero, "Info_Mod_Cadar_Eroberung_16_02_00"); //Ich stelle euch einige Novizenröcke bereit. Ihr gebt vor, Bartholo Sumpfkraut zu liefern, und geht in das Lager.
...
FUNC VOID Info_Mod_Cadar_Asylanten_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Cadar_Asylanten_16_01"); //(verdutzt) Pläne? Die Zukunft selbst wird uns zeigen, was sie für uns bereithält.
	AI_Output(self, hero, "Info_Mod_Cadar_Asylanten_16_02"); //Solange werden wir weiter ein normales Leben führen: Wir bauen Sumpfkraut an, rauchen ab und zu einen und tauschen uns über unsere spirituellen Erfahrungen aus.
	AI_Output(hero, self, "Info_Mod_Cadar_Asylanten_15_03"); //Ihr habt also nicht vor, das Minental zu verlassen?
	AI_Output(self, hero, "Info_Mod_Cadar_Asylanten_16_04"); //Nein.
	AI_Output(hero, self, "Info_Mod_Cadar_Asylanten_15_05"); //Und wie steht ihr zu den anderen Gruppierungen im Minental?
	AI_Output(self, hero, "Info_Mod_Cadar_Asylanten_16_06"); //Alle, die mit uns Handel treiben wollen, sind willkommene Gäste.
	AI_Output(self, hero, "Info_Mod_Cadar_Asylanten_16_07"); //Allerdings habe ich gehört, dass das Alte Lager viel von seinem vormaligen Status eingebüßt hat, und die Beziehungen zum neuen Lager waren nie so ausgeprägt, wie wir es uns wünschten.
	AI_Output(hero, self, "Info_Mod_Cadar_Asylanten_15_08"); //Danke für die Auskunft.
	AI_Output(self, hero, "Info_Mod_Cadar_Asylanten_16_09"); //Möge der Schläf... Mögen die Sterne dir günstig stehen.
...
FUNC VOID Info_Mod_Cadar_Woher_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Cadar_Woher_16_01"); //Bevor ich in die Barriere geworfen wurde war ich ein Feuermagier des Königs.
	AI_Output(self, hero, "Info_Mod_Cadar_Woher_16_02"); //Allerdings habe ich mich auch für andere Magierichtungen interessiert. Besonders von der Orkmagie war ich sehr angetan, doch als Gegenleistung musste ich sie unterstützen.
	AI_Output(self, hero, "Info_Mod_Cadar_Woher_16_03"); //Irgendwann haben sie dann rausgefunden, dass ich die Orks unterstütze und haben mich in die Barriere geworfen.
	AI_Output(self, hero, "Info_Mod_Cadar_Woher_16_04"); //Ich bin dann zur Bruderschaft gegangen um diese Art der Magie ebenfalls zu lernen und wurde bald darauf Cor Kaloms Stellvertreter.
	AI_Output(self, hero, "Info_Mod_Cadar_Woher_16_05"); //Nach seinem und Y'Berions Tod war ich der höhste Guru im Lager und wurde deshalb zum geistlichen Führer der Bruderschaft.
};
...
FUNC VOID Info_Mod_Cadar_Lehrer_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Cadar_Lehrer_16_01"); //Ich kann dir zeigen, wie du aus deiner Erfahrung mehr wissen schöpfst, welches du nutzen kannst um weitere Fähigkeiten zu erlernen.
...
FUNC VOID Info_Mod_Cadar_SchnellesLernen_Perk_Info()
{
	if (hero.lp > 4)
	{
		AI_Output(self, hero, "Info_Mod_Cadar_SchnellesLernen_Perk_16_00"); //Nun wirst du mehr Wissen aus deiner Erfahrung schöpfen können. Nutze dieses Wissen weise.
