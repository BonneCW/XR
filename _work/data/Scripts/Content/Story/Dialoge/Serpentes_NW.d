...
FUNC VOID Info_Mod_Serpentes_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Serpentes_Hi_03_01"); //Ich bin Serpentes, der zweithöchste Feuermagier auf Khorinis. Was willst du von mir?
};
...
{
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_00"); //Nicht so schnell, Novize. Ich habe ein Anliegen an dich.
	AI_Output(hero, self, "Info_Mod_Serpentes_Weindiebe_15_01"); //Was gibt es?
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_02"); //Wie du bestimmt weißt, hat unser Kloster neben anderen Einnahmequellen auch mit dem Verkauf von Wein sein gutes Auskommen.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_03"); //Ich weiß das besser als jeder andere, da ich mich um die finanziellen Belange des Klosters kümmere und Einnahmen und Ausgaben stets im Blick habe.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_04"); //Leider musste ich zu meinem großen Bedauern feststellen, dass es in letzter Zeit ein unschönes Missverhältnis zwischen Weinausgang und Goldeinahmen gab.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_05"); //Der Wirt Orlan hat sich zudem mehrmals über ausgebliebene Lieferungen unseres Weines beschwert.
	AI_Output(hero, self, "Info_Mod_Serpentes_Weindiebe_15_06"); //Was sagt Gorax dazu?
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_07"); //Gorax?! Der ist ein alter Säufer, der weit mehr von dem Wein genießt, als gut für ihn ist.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_08"); //Zwar hat er beteuert nur die zuverlässigsten Novizen für die Lieferungen einzusetzen, hat jedoch bereits bis mittags so viel getrunken, dass er kaum die Novizen von den Magiern unterscheiden kann.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_09"); //Mein scharfer Blick dagegen hat mir verraten, dass mehrfach Novizen das Kloster durch den Eingang verlasse haben, die vorher nicht durch diesen hineingelangt waren… und die ich vorher auch nie gesehen hatte.
	AI_Output(hero, self, "Info_Mod_Serpentes_Weindiebe_15_10"); //Was hat das zu bedeuten und wer könnte das sein?
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_11"); //Nun, da unsere magische Pforte zwar Feinde am hineinkommen, aber nicht am hinauszugehen hindert, verdächtige ich die Jünger des finsteren Gottes.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_12"); //Es würde mich in der Tat nicht überraschen, wenn diese Dämonenmagier dreist genug wären ihre Novizen zum stehlen unseres Weines hier reinzuschicken.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_13"); //Aber genug der Mutmaßungen: Ich will Gewissheit!
	AI_Output(hero, self, "Info_Mod_Serpentes_Weindiebe_15_14"); //Was soll ich machen?
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_15"); //Erst vor kurzem hat wieder einer dieser Scheinnovizen das Kloster verlassen.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_16"); //Wenn du dich beeilst, wirst du ihn vielleicht noch auf dem Weg zwischen Kloster und Taverne stellen können.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe_03_17"); //Also, du weißt, was zu tun ist.
...
{
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_00"); //Und?! Sprich schon! Was hast du über die Weindiebe zu berichten?
	AI_Output(hero, self, "Info_Mod_Serpentes_Weindiebe2_15_01"); //Du hattest Recht, es war ein Novize der Schwarzmagier.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_02"); //Ha, ich wusste es. Mein scharfer Verstand hat mich nicht getäuscht.
	AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_03"); //Hast du den Übeltäter zur Rechenschaft gezogen?
	...
	if (Npc_IsDead(Mod_7761_SNOV_Novize_NW))
	{
		...
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_05"); //Vortrefflich ... ich bin mehr als zufrieden mit dir, Novize. Das wird den Schwarzmagiern eine Lehre sein.
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_06"); //Ich bezweifle, dass sich jetzt noch irgendein Lakai des dunklen Gottes in unser Kloster wagen wird.
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_07"); //Innos Flammen sollen jeden versengen, der sich uns in den Weg stellt.
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_08"); //Hier, dieses Gold und die Spruchrollen hast du dir redlich verdient!
		...
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_09"); //Geh mit Innos Segen.
	}
	else
	{
		...
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_11"); //(erbost) Was, du hast ihn davonkommen lassen?!
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_12"); //Lieber wäre mir gewesen, du hättest ihn mit Innos’ Flammen zu Tode geläutert.
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_13"); //(nachsichtiger) Aber ich will es dir dieses eine Mal nachsehen.
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_14"); //Letztendlich hast du dafür Sorge getragen, dass die Diener der Niedertracht uns nicht mehr bestehlen werden.
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_15"); //Hier, nimm die Spruchrollen und das Gold zur Belohnung.
		...
		AI_Output(self, hero, "Info_Mod_Serpentes_Weindiebe2_03_16"); //Und nun entferne dich wieder.
	};
...
FUNC VOID Info_Mod_Serpentes_Kimon_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Serpentes_Kimon_03_01"); //So, sagt er das? Und was glaubst du, wie wenig es mich interessiert, was er sagt?
	AI_Output(self, hero, "Info_Mod_Serpentes_Kimon_03_02"); //Ich habe ihm so viel Gold gegeben, wie mir seine Trauben wert waren.
	AI_Output(hero, self, "Info_Mod_Serpentes_Kimon_15_03"); //Das war aber zu wenig.
	AI_Output(self, hero, "Info_Mod_Serpentes_Kimon_03_04"); //(wütend) Du stellst das Wort des zweithöchsten Feuermagiers von Khorinis in Frage?
	AI_Output(hero, self, "Info_Mod_Serpentes_Kimon_15_05"); //Ja, genau das mache ich.
	AI_Output(self, hero, "Info_Mod_Serpentes_Kimon_03_06"); //(laut) Verschwinde gefälligst, bevor ich dich hinauswerfen lasse!
	AI_Output(hero, self, "Info_Mod_Serpentes_Kimon_15_07"); //Nicht, bevor du diese verdammten Trauben bezahlt hast!
	AI_Output(self, hero, "Info_Mod_Serpentes_Kimon_03_08"); //So, jetzt reicht es mir.
...
FUNC VOID Info_Mod_Serpentes_KimonsGold_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Serpentes_KimonsGold_03_01"); //(wütend) Ich hab gesagt, du sollst verschwinden!
