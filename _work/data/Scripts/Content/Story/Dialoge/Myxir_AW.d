...
FUNC VOID Info_Mod_Myxir_AW_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Myxir_AW_Hi_12_06"); //Untote und der Blutkult griffen uns an. Ich hatte hier bei Greg Stellung bezogen und sie unterstützt beim Angriff.
	AI_Output(self, hero, "Info_Mod_Myxir_AW_Hi_12_07"); //Es kamen einige um, doch noch können die Piraten Stand halten. Aber für wie lange noch.
	AI_Output(self, hero, "Info_Mod_Myxir_AW_Hi_12_08"); //Das Wichtigste ist jetzt, dass wir die Garde Innos' aufhalten, ehe sie sich Innos' Schwert bemächtigen.
...
FUNC VOID Info_Mod_Myxir_AW_WhatNow_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Myxir_AW_WhatNow_12_05"); //Jedoch wirkt er nur ein mal. Die Bauanleitung muss sich irgendwo in Jhadenkar befinden. Du musst diese bergen damit wir den Dolch herstellen können.
	AI_Output(hero, self, "Info_Mod_Myxir_AW_WhatNow_15_06"); //Wo soll ich anfangen zu suchen?
	AI_Output(self, hero, "Info_Mod_Myxir_AW_WhatNow_12_07"); //Das ist das Schwierige. Jedoch gibt es im Tal mehrere Herrenhäuser der Erbauer. Am besten versuchst du dort dein Glück.

	Log_CreateTopic	(TOPIC_MOD_BEL_BELIARDOLCH, LOG_MISSION);
	B_SetTopicStatus	(TOPIC_MOD_BEL_BELIARDOLCH, LOG_RUNNING);
	B_LogEntry	(TOPIC_MOD_BEL_BELIARDOLCH, "Um die unverwundbaren Gardisten doch besiegen zu können brauche ich den Dolch Beliars. Diesen kann man nur einmal benutzen, doch er zerstört jede Rüstung mit nur einem Schlag. Das Bauanleitung dafür soll sich irgendwo in Jharkendar befinden. Myxir riet mir, die Herrenhäuser zu durchsuchen.");
};
...
{
	AI_Output(hero, self, "Info_Mod_Myxir_AW_WhatNext_15_00"); //Was ist das Nächste?
...
FUNC VOID Info_Mod_Myxir_AW_WhatLast_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Myxir_AW_WhatLast_12_01"); //Ja. Wir müssen Jhadenkar zurückgewinnen. Greg hat vor es zu befreien.
	AI_Output(self, hero, "Info_Mod_Myxir_AW_WhatLast_12_02"); //Sprich mit ihm darüber. Er hat die Pläne.
	AI_Output(hero, self, "Info_Mod_Myxir_AW_WhatLast_15_03"); //Ok, ich werde sofort aufbrechen.
...
FUNC VOID Info_Mod_Myxir_AW_BeliarDolch_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Myxir_AW_BeliarDolch_12_02"); //Aha, ein Brocken davon und ein Brocken hiervon. Hmm, nun, du hast nicht zufällig je einen Brocken magisches und schwarzen Erz bei dir?
};
