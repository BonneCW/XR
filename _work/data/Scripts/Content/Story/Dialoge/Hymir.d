...
{
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_00"); //Du bist also derjenige, von dem hier alle reden. Du hast uns eine Menge Arbeit abgenommen.
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_01"); //Aber noch können wir kein Erz fördern, da in der Mine diese merkwürdigen Steinmonster sind.
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_02"); //Wir brauchen noch etwas Vorbereitungszeit, dann werden wir die Mine stürmen.
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_03"); //Komm also später wieder.
...
{
	AI_Output(self, hero, "Info_Mod_Hymir_MineLos_16_00"); //Wir sind nun soweit. In der Mine tummeln sich nach wie vor die Steinmonster.
	AI_Output(self, hero, "Info_Mod_Hymir_MineLos_16_01"); //Es liegt nun an dir diese zu vernichten und die Mine zu erschließen.
	AI_Output(self, hero, "Info_Mod_Hymir_MineLos_16_02"); //Geh zu Granmar, er wartet in der Mine auf dich. Er wird dir bei deiner Aufgabe helfen.
...
FUNC VOID Info_Mod_Hymir_MineFertig_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_01"); //Schön zu hören, damit hast du es geschafft, in die kleine Reihe der Erzfürsten aufgenommen zu werden.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_02"); //Und glaub mir, dass haben nur sehr wenige in so kurzer Zeit geschafft.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_03"); //Du bestitzt jetzt somit 10% des Einkommens der Mine. Die Summe kannst du dir jede Woche bei Ole abholen.

	B_GiveInvItems	(self, hero, ItMi_Nugget, 5);

	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_04"); //Natürlich bekommst du jetzt auch eine neue Rüstung.

	CreateInvItems	(hero, ItAr_Erzbaron_Blau, 1);

	B_ShowGivenThings	("Königliche Erzfürstenrüstung erhalten");

	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_05"); //Ach, übrigens, der Paladin war kurz vor dir bei mir. Er meinte, er verdanke dir sein Leben.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_06"); //Gemeinsam mit einer Miliz-Truppe wurde er von Lord Andre persönlich ins Minental geschickt. Sie sollen dort die Orks ausspionieren.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_07"); //Lord Andre wollte übrigens auch noch mal mit dir reden, aber ich denke das dürfte noch Zeit haben.
...
FUNC VOID Info_Mod_Hymir_Trent_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Trent_16_01"); //Was ... wir kommen ja noch nicht mal mit der einen nach. Vielleicht schickt uns Lord Andre ja jetzt ein paar neue Leute.
	AI_Output(self, hero, "Info_Mod_Hymir_Trent_16_02"); //Egal, das muss gefeiert werden. Geh zu Melcador und lass dir eine Erzwaffe und ein frisches Bier geben.
	AI_Output(self, hero, "Info_Mod_Hymir_Trent_16_03"); //Für dich ist jetzt nicht mehr zu tun. Lehn dich zurück und genieß das Leben, so wie ich!
...
{
	AI_Output(self, hero, "Info_Mod_Hymir_Eroberung_06_16_00"); //... und wir in das Lager ein ...
};
...
{
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_00"); //Eine Botschaft von Hagen? Ich bin gespannt, was der Aufschneider von mir möchte.

	B_GiveInvItems	(hero, self, ItWr_HagenForHymir, 1);

	B_UseFakeScroll();

	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_01"); //Ha, dass ich nicht lache. Jetzt, wo ihm der Arsch brennt sollen wir für ihn die Kohlen aus dem Feuer holen.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_02"); //Alles was recht ist. Und wie ... du bist der neue Hauptmann? Das wird ja immer schöner.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_03"); //Was ist mit Andre ... dem einzigen, der ein bisschen Anstand und Rückgrat hatte?
	AI_Output(hero, self, "Info_Mod_Hymir_Treue_15_04"); //Er ist tot. Er opferte sich für die Stadt, als diese bedroht wurde.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_05"); //(schockiert) Was ... er ist tot? Für die Stadt ...?
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_07"); //Geopfert? (bäumt sich auf) Erzähl mir doch nichts.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_08"); //Hagen hat ihn bestimmt als Bauernopfer benutzt, um sich selbst anschließend als edlen Retter hinzustellen.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_09"); //Und jetzt sollen auch wir für ihn den Kopf hinhalten. Da hört sich doch alles auf.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_10"); //Nicht mal den Anstand selbst zu erscheinen hat er, sondern schickt einen Laufburschen.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_11"); //Nein ... und jetzt geh mir besser aus den Augen.
...
FUNC VOID Info_Mod_Hymir_Treue2_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_00"); //(verblüfft) Ist es möglich? Hagen, du hier!?
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_02"); //Ich ... (fängt sich wieder) ... ja, ich habe dein Schreiben empfangen.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_03"); //Und auch, wenn du hier vor mir stehst, sehe ich keinerlei Veranlassung dazu, darauf einzugehen.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_04"); //Du weißt selbst am besten, dass wir unseren Aufenthalt in der Kolonie dir zu verdanken haben.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_05"); //(streng) Und dass Andre unter deinem Kommando sein Leben ließ, macht den Sachverhalt nicht gerade besser.
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_08"); //(liest) Roboter einzuschmelzen […] mordender Maschine […] denen sie das Leben nahm […] niemand ohne Schuld ist ...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_09"); //Hmm, dies ist tatsächlich Andres Handschrift ... und es scheint zu stimmen, was ihr mir über sein Schicksal berichtet habt.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_10"); //Im Lichte dieser Ereignisse will ich mich nicht mehr grundsätzlich gegen eine Zusammenarbeit mit der Stadt stellen.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_11"); //Ich tue es aber gewiss nicht, um dir einen Gefallen zu tun, Hagen, sondern um des Beispieles Willen, das Andre uns gegeben hat.
