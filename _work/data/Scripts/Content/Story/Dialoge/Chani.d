...
{
	AI_Output(self, hero, "Info_Mod_Chani_Hi_19_00"); //Und Süßer, wie wäre es mir uns beiden?
...
FUNC VOID Info_Mod_Chani_Woher_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Chani_Woher_19_01"); //Hör mal Süßer, wenn du mit mir reden willst, musst du mich zumindest auf ein Getränk einladen ... aber kein billiges Bier oder irgendeinen Seemannsfusel.
};
...
FUNC VOID Info_Mod_Chani_Woher_Story(var C_Item itm)
{
	...
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_00"); //Das hört sich doch gut an.

	B_UseItem	(self, itm);

	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_01"); //(leicht angetrunken) Sehr schön, da wird’s mir doch gleich angenehm warm um die Brust.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_02"); //Aber wo waren wir stehen geblieben ... ja, genau, die verlauste Kolonie. Daran möchte ich gar nicht denken.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_03"); //Dauernd in dieses eklige kalte Gemäuer eingepfercht, mit diesem säuselnden Tattergreis. Der Schläfer erwache ... so ein Schwachsinn.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_04"); //Habe die Gelegenheit genutzt, als der Alte abgekratzt ist, und mich kurz darauf mit Viran und Harlok hierher durchgeschlagen.
	AI_Output(hero, self, "Info_Mod_Chani_Woher_Story_15_05"); //Naja, das ist hier aber auch nicht gerade ein schöner Ort.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_06"); //Tausend mal besser als dort.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_07"); //Hier habe ich wenigstens meine Freiheit, kann wie vor der Zeit in der Kolonie meinen Körper verkaufen, wie es mir gerade gefällt, habe Spaß und vor allem klingendes Geld.
	AI_Output(hero, self, "Info_Mod_Chani_Woher_Story_15_08"); //Bist du deshalb in der Kolonie gelandet?
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_09"); //Ja, genau. Der dreckige Stadthalter hat mich denunziert, als ich auf den südlichen Inseln meinem Gewerbe nachging.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_10"); //Dabei war er einer meiner besten Kunden.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_11"); //Hat wohl eine Prämie bei diesem Schwein Rhobar bekommen dafür, dass sie ein weiteres Weibsbild für die Kolonie gefunden haben.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_12"); //Vielleicht war ich ihm auch zu gefährlich geworden, für seinem untadeligen Ruf ... Scheinheiliges Pack.
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_13"); //Wenn ich könnte, würde ich ein Rasiermesser nehmen und ihnen ihre ...
	...
	AI_Output(self, hero, "Info_Mod_Chani_Woher_Story_19_15"); //Ja, genug geredet. Ich trinke noch mal einen Schluck, um in Stimmung zu kommen.
