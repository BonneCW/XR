...
{
	AI_Output(self, hero, "Info_Mod_Alchemist01_Hi_16_00"); //He, dich habe ich hier noch nie gesehen, Bruder. Bist du gerade von draußen reingekommen?
	AI_Output(hero, self, "Info_Mod_Alchemist01_Hi_15_01"); //Ja, das bin ich.
	AI_Output(self, hero, "Info_Mod_Alchemist01_Hi_16_02"); //Hast du zufällig einige Kräuter bei dir?
	AI_Output(self, hero, "Info_Mod_Alchemist01_Hi_16_03"); //Durch die Belagerung herrscht hier nämlich Mangel an allem und es sind vor allem die Tränke, die uns zu dieser schweren Stunde Kraft geben den Angriffen des Feindes zu widerstehen.
	AI_Output(self, hero, "Info_Mod_Alchemist01_Hi_16_04"); //Nur gehen uns eben langsam die Zutaten für diese aus.
	AI_Output(self, hero, "Info_Mod_Alchemist01_Hi_16_05"); //Wenn du mir also eine Hand voll verschiedener Kräuter besorgen könntest -  sagen wir mal von jedem ein dutzend Stück – wären wir zumindest von dieser Sorge für die nächsten Wochen befreit.
...
{
	AI_Output(self, hero, "Info_Mod_Alchemist01_MangelQuest_Fertig_16_00"); //Bei Innos, du musst Stunden damit zugebracht haben die ganzen Kräuter zu sammeln. Damit hat sich das mit dem Mangel an Zutaten erst mal erübrigt.
	AI_Output(self, hero, "Info_Mod_Alchemist01_MangelQuest_Fertig_16_01"); //Hier, diese Tränke werden dir bei deinen weiteren Reisen sicher von Nutzen sein.

	B_ShowGivenThings	("Elixier des Lebens, 2 Gegengifte und Trank der Genesung erhalten");

	AI_Output(self, hero, "Info_Mod_Alchemist01_MangelQuest_Fertig_16_02"); //Innos behüte dich.
...
FUNC VOID Info_Mod_Alchemist01_Fake_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alchemist01_Fake_16_01"); //Ein Magier war eben noch hier. Er hat gesagt, er wolle zu den Übungsräumen.
