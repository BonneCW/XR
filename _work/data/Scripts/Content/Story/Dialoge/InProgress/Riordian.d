...
{
	AI_Output(self, hero, "Info_Mod_Riordian_Hi_23_00"); //Du traust dir ganz schön was zu.
	AI_Output(self, hero, "Info_Mod_Riordian_Hi_23_01"); //Hätte nicht gedacht dich nochmal wieder zu sehen.
};
...
{
	AI_Output(self, hero, "Info_Mod_Riordian_IrrlichtBekommen_23_00"); //Ich habe das hier im Tempel entdeckt.

	B_GiveInvItems	(self, hero, ItAm_Addon_WispDetector, 1);

	AI_Output(self, hero, "Info_Mod_Riordian_IrrlichtBekommen_23_01"); //Es scheint, als könne man damit ein Irrlicht rufen, welches einem Gegenstände sucht.
	AI_Output(self, hero, "Info_Mod_Riordian_IrrlichtBekommen_23_02"); //Dir ist es bestimmt von größerem Nutzen als mir.
	AI_Output(self, hero, "Info_Mod_Riordian_IrrlichtBekommen_23_03"); //Ich werde versuchen etwas mehr darüber herauszufinden, vielleicht lässt sich das Aufgabenfeld des Irrlichtes erweitern, so dass es mehr Gegenstände finden kann oder sogar für dich kämpfen.
};
...
FUNC VOID Info_Mod_Riordian_PortalOeffnen_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Riordian_PortalOeffnen_23_01"); //Ja. Ich hab die Sprache der Erbauer durch die Schriften und Zeichen an den Wänden lernen können.
	AI_Output(self, hero, "Info_Mod_Riordian_PortalOeffnen_23_02"); //Da du das Portal als erster durchschreiten wirst, sobald es geöffnet ist, solltest du zumindest die Grundlagen dieser Sprache beherrschen.
	AI_Output(self, hero, "Info_Mod_Riordian_PortalOeffnen_23_03"); //Also höre gut zu.

	hero.lp += 5;

	B_TeachPlayerTalentForeignLanguage (self, hero, LANGUAGE_1);

	AI_Output(self, hero, "Info_Mod_Riordian_PortalOeffnen_23_04"); //Also, es es ganz einfach. Ein G ist ein O, ein T ist ein H ist ein I ist ein C.
	AI_Output(self, hero, "Info_Mod_Riordian_PortalOeffnen_23_05"); //Wenn du das mal kapiert hast, ist der Rest eigentlich logisch ...
...
{
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht_23_00"); //Adanos sei dank. Wärst du nicht gekommen, wären wir alle verloren gewesen.
...
FUNC VOID Info_Mod_Riordian_TempelDicht2_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_01"); //Es war ein Tag wie jeder. Wir gingen alle unserem Tagwerk nach, bis plötzlich Kampfgeräusche von draußen zu uns reindrangen.
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_02"); //Wir wollten nachsehen, was los war, doch dann telepotierten sich fünf extrem mächtige Gardisten in den Portalraum und fegten alles nieder und nahmen die anderen Wassermagier gefangen.
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_03"); //Ich entkam im letzen Moment nach draußen und dann versiegelte sich der Eingang. Dann kamst du zu uns und hast geholfen die Gegner zu besiegen.
	...
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_05"); //Der Eingang ist magisch verschlossen. man bekommt ihn nicht ohne Weiteres auf.
	AI_Output(hero, self, "Info_Mod_Riordian_TempelDicht2_15_06"); //Was muss ich tun um in den Tempel zu gelangen?
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_07"); //Nun, wenn ich mich recht entsinne gibt es eine Art magischen Schlüssel um die Tür zu öffnen. Jedoch habe ich keine Ahnung, wo sich dieser befindet.
	AI_Output(hero, self, "Info_Mod_Riordian_TempelDicht2_15_08"); //Weißt du wirklich nicht wo der Schlüssel sein kann?
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_09"); //(nachdenklich) Hmmmm. Den Standort kennen nur Vatras und Saturas. Gehe am besten zu vatras in die Stadt und frage ihn um Rat.
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_10"); //Beeil dich aber, die Zeit drängt!
	AI_Output(hero, self, "Info_Mod_Riordian_TempelDicht2_15_11"); //Ich werde sofort aufbrechen.
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht2_23_12"); //Viel Glück!
...
FUNC VOID Info_Mod_Riordian_TempelDicht3_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht3_23_01"); //(erfreut) Sehr gut. Gib mir den Schlüssel, damit ich den Eingang öffnen kann.
	AI_Output(hero, self, "Info_Mod_Riordian_TempelDicht3_15_02"); //Hier hast du ihn.

	B_GiveInvItems	(hero, self, ItKe_MagicTempelKey, 1);

	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht3_23_03"); //Geh nun beiseite, damit ich den Eingang öffnen kann.
...
{
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht4_23_00"); //Das Tor öffnet sich. Wir müssen die Anderen retten.
	AI_Output(hero, self, "Info_Mod_Riordian_TempelDicht4_15_01"); //Ich gehe vor. Sorge dafür, dass uns niemand von hinten überrascht.
	AI_Output(self, hero, "Info_Mod_Riordian_TempelDicht4_23_02"); //Gut, geh voran.
