...
FUNC VOID Info_Mod_Nefarius_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_Hi_16_01"); //Fortschritte schon, aber wir sind noch weit davon entfernt, etwas zu verstehen.
	AI_Output(self, hero, "Info_Mod_Nefarius_Hi_16_02"); //Niemand von uns kennt diese Sprache. Ich versuche, die Bedeutung der Zeichen zu verstehen. Dies funktioniert jedoch nur mit vielen Vergleichen und Vermutungen.
	AI_Output(self, hero, "Info_Mod_Nefarius_Hi_16_03"); //Es kann also noch dauern, bis wir erste Ergebnisse vorweisen können.
};
...
{
	AI_Output(self, hero, "Info_Mod_Nefarius_Saturas_16_00"); //Ich hab etwas neues herausgefunden. Es scheint einen Ring zu geben der in eine kleine Öffnung in der Wand neben dem Portal passt.
	AI_Output(self, hero, "Info_Mod_Nefarius_Saturas_16_01"); //Den Inschriften Zufolge wurde dieser Ring zur Sicherheit in vier gleiche Teile geteilt und in allen vier Himmelrichtungen versteckt.
	AI_Output(self, hero, "Info_Mod_Nefarius_Saturas_16_02"); //Bevor du dich auf die Suche machst solltest du noch mit Merdarion reden.
...
FUNC VOID Info_Mod_Nefarius_PortalDicht_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_PortalDicht_16_01"); //Nun, hier steht eine Geschichte geschrieben. Demnach hat ein Gott mit Namen Radanos vor langer Zeit im alten Tal, genannt Jharkendar, geherrscht.
	AI_Output(self, hero, "Info_Mod_Nefarius_PortalDicht_16_02"); //Er war gütig und weise und schien mit den Bewohnern spirituell viel enger verbunden, als wir es von den drei Gottheiten heute kennen.
	AI_Output(hero, self, "Info_Mod_Nefarius_PortalDicht_15_03"); //Ein anderer Gott? Erzähl weiter ...
	AI_Output(self, hero, "Info_Mod_Nefarius_PortalDicht_16_04"); //Nun, es schien eine Art von Unheil oder Konflikt gegeben zu haben. Näheres konnte ich auch nicht erfahren.
	AI_Output(self, hero, "Info_Mod_Nefarius_PortalDicht_16_05"); //Auf jeden Fall scheint ganz plötzlich die Präsenz dieses Gottes zu enden ... und mit dieser Katastrophe beinahe auch alles Leben in Jharkendar ausgelöscht.
	AI_Output(self, hero, "Info_Mod_Nefarius_PortalDicht_16_06"); //Scheinbar sind nur vier der Erbauer entkommen und haben das Portal verschlossen. Sie wollten aus ihren Fehlern der Vergangenheit lernen.
	AI_Output(self, hero, "Info_Mod_Nefarius_PortalDicht_16_07"); //Den Schlüssel haben sie zerbrochen und geschworen, sich niemals wieder zu begegnen, um die Vergangenheit für immer zu begraben.
	AI_Output(self, hero, "Info_Mod_Nefarius_PortalDicht_16_08"); //Dann sind sie in verschiedene Richtungen aufgebrochen, damit die Stücke des Schlüssel voneinander getrennt sind.
...
FUNC VOID Info_Mod_Nefarius_VierErbauer_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_01"); //Nichts genaues. Außerdem musst du bedenken, dass sich das Gebiet hier in den letzten Jahrhunderten auch verändert haben kann.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_02"); //Ich will dir jedoch so viel sagen, wie mir möglich ist.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_03"); //Einer der Erbauer, ein Priester, wollte in den Westen aufbrechen, um sich dort eine Residenz zu erschaffen und die Weisheit vom Ur-Adanos zu predigen.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_04"); //Leider steht hier nicht, was aus ihm geworden ist. Vielleicht solltest du dich mal in Khorinis umhören. Vielleicht kann der Statthalter in seinen alten Büchern darüber etwas in Erfahrung bringen.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_05"); //Ein anderer ist in den Süden gegangen. Er wollte dort sein Glück in den Minen suchen.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_06"); //Den Schriften nach war er allerdings nur ein Mitglieder der Kaste der Heiler.
	AI_Output(hero, self, "Info_Mod_Nefarius_VierErbauer_15_07"); //Und was heißt das?
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_08"); //Nun, zur damaligen Zeit gab es noch keine Handels- und Transportroute zwischen dem Minental und Khorinis.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_09"); //Daher dürften dort jede Menge gefährliche Monster unterwegs gewesen sein. Ich glaube kaum, dass ein Heiler da sehr weit gekommen ist.
	AI_Output(hero, self, "Info_Mod_Nefarius_VierErbauer_15_10"); //Klingt einleuchtend.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_11"); //Ein weiterer Erbauer, ein Krieger, wollte sich auf Khorinis niederlassen, eine Familie gründen und in Frieden leben.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_12"); //Sein Name war Onar. Vielleicht hat gibt es ja eine Verbindung zum Großbauer ...
	AI_Output(hero, self, "Info_Mod_Nefarius_VierErbauer_15_13"); //Ich werde mir das mal ansehen.
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_14"); //Der letzte der Überlebenden war ein Totenwächter, der wohl irgendwo im Norden von Khorinis zu finden sein muss.
	AI_Output(hero, self, "Info_Mod_Nefarius_VierErbauer_15_15"); //Zumindest das, was von ihm übrig ist ...
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_16"); //Natürlich, es wäre erstaunlich, wenn er selber noch Leben würde. (lacht)
	AI_Output(self, hero, "Info_Mod_Nefarius_VierErbauer_16_17"); //Schau dich mal im Norden von Khorinis um, vielleicht findest du dort einen Ort, der einem Totenwächter würdig ist.
...
FUNC VOID Info_Mod_Nefarius_PriesterOrnament_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_PriesterOrnament_16_01"); //In Khorata? Nun ja, jetzt haben wir es ja. Hier ist deine Belohnung.
...
FUNC VOID Info_Mod_Nefarius_HeilerOrnament_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_HeilerOrnament_16_01"); //Sehr gut. Hier hast du eine Belohnung.
...
FUNC VOID Info_Mod_Nefarius_KriegerOrnament_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_KriegerOrnament_16_01"); //Schön, wo hast du es gefunden?
	AI_Output(hero, self, "Info_Mod_Nefarius_KriegerOrnament_15_02"); //Es war in einer Truhe von Onar, dem Grossbauern.
	AI_Output(self, hero, "Info_Mod_Nefarius_KriegerOrnament_16_03"); //Ah ja, also waren die beiden tatsächlich verwandt.
	AI_Output(self, hero, "Info_Mod_Nefarius_KriegerOrnament_16_04"); //Hier ist deine Belohnung.
...
FUNC VOID Info_Mod_Nefarius_TotenwaechterOrnament_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_TotenwaechterOrnament_16_01"); //Und welcher Ort war des Totenwächters würdig?
	AI_Output(hero, self, "Info_Mod_Nefarius_TotenwaechterOrnament_15_02"); //Der Steinkreis in der Nähe des schwarzen Trolls.
	AI_Output(self, hero, "Info_Mod_Nefarius_TotenwaechterOrnament_16_03"); //Der Sonnenkreis also, interessant.
	AI_Output(self, hero, "Info_Mod_Nefarius_TotenwaechterOrnament_16_04"); //Sehr gut. Hier ist eine kleine Belohnung für dich.
...
FUNC VOID Info_Mod_Nefarius_Komplett_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_Komplett_16_01"); //Nun, ich werde die Teile jetzt zusammensetzen. Es wird allerdings ein wenig dauern.
	AI_Output(self, hero, "Info_Mod_Nefarius_Komplett_16_02"); //Geh solange mal zu Riordian, ich glaube, er wollte dir etwas zeigen.
...
FUNC VOID Info_Mod_Nefarius_NTag_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Nefarius_NTag_16_01"); //Ja, hier ist der Ring.

	B_GiveInvItems	(self, hero, ItMi_PortalRing_Addon, 1);

	AI_Output(self, hero, "Info_Mod_Nefarius_NTag_16_02"); //Setze ihn in die vorgesehen Öffnung und durchschreite dann das Portal. Wir werden dir dann folgen. Du kannst dich solange ein wenig umsehen.
