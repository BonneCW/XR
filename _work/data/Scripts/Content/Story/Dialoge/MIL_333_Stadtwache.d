...
{
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_06_00"); //Du willst hier rein, nehme ich an?
...
FUNC VOID Info_Mod_Mil_333_Hi_B()
{
	...
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_B_06_00"); //(lacht) Oh, das macht mir ja Angst. Na los, komm doch, wenn du dich traust!
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_B_06_01"); //Und solltest du dir jetzt doch in die Hosen scheißen, dann versuch's lieber auf der anderen Seite bei Garond. Der ist heute für Verbrechergesocks zuständig.
...
FUNC VOID Info_Mod_Mil_333_Hi_A()
{
	...
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_A_06_01"); //Du bist wohl einer der Sträflinge, die aus dem Minental ausgebrochen sind?
	AI_Output(hero, self, "Info_Mod_Mil_333_Hi_A_15_02"); //Kann sein.
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_A_06_03"); //Bist du. Ich kenne jeden, der in die Stadt ein- und ausgeht. Aber dich habe ich noch nie gesehen.
	AI_Output(hero, self, "Info_Mod_Mil_333_Hi_A_15_04"); //Darf ich denn jetzt vorbei?
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_A_06_05"); //Nicht bei mir. Garond will euch Kuppelbewohner selbst in Augenschein nehmen, bevor er euch reinlässt.
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_A_06_06"); //Du wirst dich zum anderen Tor bemühen müssen.
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi_A_06_07"); //Aber versuch am besten, Garond gegenüber weniger ... frech zu sein. Er ist ein ziemlicher Kotzbrocken, und er kann euch so gar nicht leiden.
...
{
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi2_06_00"); //Du willst hier rein, nehme ich an?
	AI_Output(hero, self, "Info_Mod_Mil_333_Hi2_15_01"); //Sieht so aus.
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi2_06_02"); //Ich habe dich noch nie gesehen.
	...
	AI_Output(self, hero, "Info_Mod_Mil_333_Hi2_06_04"); //Nee, lass mal, ich hab heute noch was Besseres vor. Na gut, komm rein.
};
...
{
	AI_Output(self, hero, "Info_Mod_Mil_333_TomKraut_06_00"); //Ohh, mein Kopf ... er fühlt sich an wie leer gesaugt.
...
{
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_00"); //Ahh, schon wieder einer dieser verrückten Ausflügler.
	AI_Output(hero, self, "Info_Mod_Mil_333_Daemonisch_15_01"); //Was meinst du mit verrückt und dem ganzen?
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_02"); //Naja, ich meine die ganzen Bürger die immer wieder Ausflüge aus der Stadt machen.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_03"); //In letzter Zeit sind es verdammt viele geworden. Und verrückt eben das, was so in letzter Zeit alles in der Nähe der Stadttore passiert.

	B_Say	(hero, self, "$WASMEINSTDU");

	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_04"); //Naja, zunächst die Banditen in dem kleinen Tal vor der Stadt.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_05"); //Ein Bürger hat sie entdeckt und einige Paladine und Stadtwachen abends dorthin geführt.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_06"); //Der Paladin Tengron, der eigentlich in der Stadt wache halten sollte, begann sich nach einiger Zeit Sorgen zu machen und lief später nach.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_07"); //Es dauerte gar nicht lange, da kam er schreiend zurück und berichtete etwas von finsteren Schatten.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_08"); //Jaja, haha, Schatten in der Nacht, dem muss sein heiliges Licht zu sehr auf die Rübe geschienen haben.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_09"); //Von den anderen, die einige Zeit später zurückkamen, erfuhren wir, dass er in Tobsucht verfiel und jeden Angriff. Seitdem sitzt er in der Arrestzelle ...
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_10"); //Und dann noch Brian. Kam vor gar nicht langer Zeit aus der Stadt gestürmt, stammelte irgendetwas von Bedrohung der Stadt und finsteren Schatten.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_11"); //Ist dann Richtung Orlan zwischen den Bäumen verschwunden.
	AI_Output(hero, self, "Info_Mod_Mil_333_Daemonisch_15_12"); //Ähh, ist das nicht irgendwie ein Grund für dich um dir Sorgen zu machen?
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_13"); //Wieso? Die Paladine durchforsten so oft abends die dunklen Wälder, dass man sich nicht sicherer fühlen kann.
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch_06_14"); //Die beiden Paladine die jedoch vor ein paar Tagen in das kleine Waldstück nördlich von uns gegangen sind, lassen sich aber echt Zeit, alles nach Banditen zu durchsuchen.

	B_LogEntry	(TOPIC_MOD_DAEMONISCH, "Mann, die Torwache scheint sich ja auch über gar nichts zu wundern. Ein Bürger hatte Paladine und Stadtwachen abends ins kleine Tal geführt, wo sich angeblich Banditen befanden. Tengron, der ihnen etwas später nachging, kam verstört zurückgelaufen und berichtete von finsteren Schatten. Die Begleiter, die später zurückkamen, behaupteten er hätte sie angegriffen und sei verrückt geworden. Seit dem sitzt er im Gefängnis. Brian kam bei anderer Gelegenheit ebenfalls verängstigt zum Stadttor gelaufen und murmelte von schwarzen Schatten und Bedrohung der Stadt. Zuletzt erfuhr ich noch, dass zwei Paladine im nördlichen Waldstück beim Osttor verschwunden sind.");
...
{
	AI_Output(self, hero, "Info_Mod_Mil_333_Daemonisch2_06_00"); //Halt, erst den ¼ Klosterwein trinken
...
FUNC VOID Info_Mod_Mil_333_Mario_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Mil_333_Mario_06_01"); //(lacht) Stadtmauern sind so geplant, dass niemand rüber klettern kann.
	AI_Output(hero, self, "Info_Mod_Mil_333_Mario_15_02"); //Ach so. Da hinten tun das aber zwei Kerle.
	AI_Output(self, hero, "Info_Mod_Mil_333_Mario_06_03"); //Was?! Wo?
	...
	AI_Output(self, hero, "Info_Mod_Mil_333_Mario_06_06"); //Bei Innos! Immer wenn ICH Dienst habe!
...
{
	AI_Output(self, hero, "Info_Mod_Mil_333_HaradLehrling_06_00"); //Stopp. Du bist doch der Lehrling von Harad, oder?
	AI_Output(hero, self, "Info_Mod_Mil_333_HaradLehrling_15_01"); //Ja, schon ...
	AI_Output(self, hero, "Info_Mod_Mil_333_HaradLehrling_06_02"); //Auf Anweisung Tengrons müssen wir dich durchsuchen. Einen kleinen Moment, bitte ...

	AI_PlayAni	(self, "T_PLUNDER");

	AI_Output(self, hero, "Info_Mod_Mil_333_HaradLehrling_06_03"); //Um dieses Waffenbündel ging es. Damit verstößt Harad gegen eine Bestimmung Tengrons. Wir müssen es an uns nehmen.

	B_GiveInvItems	(hero, self, ItMi_Waffenbuendel, 1);

	AI_Output(self, hero, "Info_Mod_Mil_333_HaradLehrling_06_04"); //Dass Harad versucht, es durch dich zu den Söldnern zu schmuggeln, wird ernsthafte Konsequenzen für ihn haben.
...
{
	AI_Output (self, other,"DIA_Mil_333_Stadtwache_SecondWarn_06_00"); //Kein Durchgang für dich, habe ich doch schon erwähnt.
