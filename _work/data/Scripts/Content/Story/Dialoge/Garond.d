...
{
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_00"); //Wo kommst du her?
	AI_Output(hero, self, "Info_Mod_Garond_Hi_15_01"); //Von außerhalb der Stadt.
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_02"); //Bist du ein Flüchtling der Kolonie? (mustert den Helden) Du bist ein Flüchtling der Kolonie.
	AI_Output(hero, self, "Info_Mod_Garond_Hi_15_03"); //Und wenn es so wäre?
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_04"); //Wir dulden hier keine Verbrechen, klar?
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_05"); //Die Lords haben gnädig verfügt, dass euch Halunken eine zweite Chance gewährt sein soll, aber wenn es nach mir ginge, würdet ihr alle morgen am Strick baumeln.
	AI_Output(hero, self, "Info_Mod_Garond_Hi_15_06"); //Dann bin ich froh, dass es nicht nach dir geht.
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_07"); //(knurrt) Sieh dich bloß vor. Ich werde ein Auge auf dich haben.
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_08"); //Wenn du dich nicht benimmst, sprechen wir uns wieder.
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_09"); //Und lass dir bloß nicht einfallen, hier auch mit Märchen hausieren zu gehen, wie du den Schläfer besiegt hast.
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_10"); //Von solchen Typen haben wir schon genug.
	AI_Output(hero, self, "Info_Mod_Garond_Hi_15_11"); //Wer behauptet denn so etwas von sich?
	AI_Output(self, hero, "Info_Mod_Garond_Hi_04_12"); //Monty im Hafenviertel. Ist der letzte, den ich noch nicht eigenhändig aus der Stadt geworfen habe.
...
FUNC VOID Info_Mod_Garond_Argez_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Argez_04_01"); //Du meinst wohl den Tätowierten? Mir hat seine Fresse einfach nicht gefallen. Der hätte bestimmt Ärger gemacht.
...
FUNC VOID Info_Mod_Garond_Argez_B()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Argez_B_04_01"); //(erregt) Glaubst du, mit so einer ... Beleidigung meiner Werte kannst du mich umstimmen?
	AI_Output(self, hero, "Info_Mod_Garond_Argez_B_04_02"); //Pass auf, dass du nicht gleich endgültig vor den Toren landest!
...
FUNC VOID Info_Mod_Garond_Argez_A()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Argez_A_04_01"); //(lacht) Das wird ja immer schöner! Ist wohl dein Kollege, was?
	AI_Output(self, hero, "Info_Mod_Garond_Argez_A_04_02"); //Mein letztes Wort dazu ist gesprochen: Er kommt nicht rein!
...
{
	AI_Output(self, hero, "Info_Mod_Garond_Argez2_04_00"); //Versucht ihr nun doch, gegen meine Anweisungen zu verstoßen? (brüllt) Raus mit euch!
...
FUNC VOID Info_Mod_Garond_Zurechtfinden_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Zurechtfinden_04_01"); //(gehässig) Ziemlich schlecht, nehme ich mal an, es sei denn, du bist schon mal hier gewesen.
	AI_Output(self, hero, "Info_Mod_Garond_Zurechtfinden_04_02"); //Außer dass wir Paladine hier nun für Ruhe und Ordnung sorgen, ändert sich nämlich in dieser Scheißstadt nichts.
};
...
FUNC VOID Info_Mod_Garond_News_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_News_04_01"); //Allerdings! Seit ich Wache stehen muss, bricht eine Katastrophe nach der anderen über uns herein.
	AI_Output(self, hero, "Info_Mod_Garond_News_04_02"); //Erst terrorisiert uns eine Räuberbande, und wir haben nicht genügend Mann, um deren Nest auszuräuchern, dann kommt ihr Verbrecher, und jetzt heulen uns die Bauern die Ohren voll wegen so eines finsteren Kapuzenmannes, der sie ärgern soll.
	AI_Output(self, hero, "Info_Mod_Garond_News_04_03"); //Aber was soll so ein einzelner Knabe schon anrichten. (lacht dreckig)
	AI_Output(hero, self, "Info_Mod_Garond_News_15_04"); //Noch nie etwas von Magie gehört?
	AI_Output(self, hero, "Info_Mod_Garond_News_04_05"); //Schnauze!
};
...
FUNC VOID Info_Mod_Garond_Miesepeter_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Miesepeter_04_01"); //Dass ich hier vorm Dorf das ganze Gesindel einführen darf, hat mir kräftig die Laune verdorben.
	AI_Output(self, hero, "Info_Mod_Garond_Miesepeter_04_02"); //Lord Hagen weiß auch nicht mehr seine guten von den schlechten Männern zu unterscheiden.
	AI_Output(hero, self, "Info_Mod_Garond_Miesepeter_15_03"); //Du hältst wohl nicht sehr viel von Lord Hagen.
	AI_Output(self, hero, "Info_Mod_Garond_Miesepeter_04_04"); //Wag es ja nicht, den Namen dieses verdienstvollen Mannes durch den Dreck zu ziehen!
	AI_Output(self, hero, "Info_Mod_Garond_Miesepeter_04_05"); //Nur weil er momentan zu zögerlich ist, heißt das nicht, dass ich nicht hundertprozentig hinter ihm stehe!
};
...
{
	AI_Output(self, hero, "Info_Mod_Garond_Verbrecher_04_00"); //Warte mal, du Verbrecher!
	AI_Output(self, hero, "Info_Mod_Garond_Verbrecher_04_01"); //Wie mir zu Ohren gekommen ist, kannst du dich in der Stadt nicht an einfache Regeln halten.
	AI_Output(self, hero, "Info_Mod_Garond_Verbrecher_04_02"); //Wie wär's mal mit einer kleinen Lektion?
...
FUNC VOID Info_Mod_Garond_Blutkelch_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Blutkelch_04_01"); //Was soll der Scheiß?
	...
	AI_Output(self, hero, "Info_Mod_Garond_Blutkelch_04_04"); //Du sagst es. Ein Dämon. Den greif ich mir. Der macht die Gegend nicht länger unsicher!
...
{
	AI_Output(self, hero, "Info_Mod_Garond_Mario_04_00"); //Ich will dich noch mal ausdrücklich warnen.
	AI_Output(self, hero, "Info_Mod_Garond_Mario_04_01"); //Du wurdest zusammen mit Mario gesehen, und es heißt, dass du ihm hilfst, seine Prüfungen zu bestehen.
	AI_Output(self, hero, "Info_Mod_Garond_Mario_04_02"); //Ich ... Wir dulden ... so etwas nicht. Ein Milizionär muss die ihm auferlegten Bürden allein tragen, kapiert?
	AI_Output(self, hero, "Info_Mod_Garond_Mario_04_03"); //Also halte dich in Zukunft besser fern von ihm. Wäre besser für euch beide.
...
{
	AI_Output(self, hero, "Info_Mod_Garond_Mario2_04_00"); //(gelassen) Ich habe ihn nicht provoziert, sondern diesen Querkopf nur in seine Schranken verwiesen.
	AI_Output(self, hero, "Info_Mod_Garond_Mario2_04_01"); //Wenn er mich deswegen zum Duell herausfordert, soll er seine gerechte Strafe erhalten.
	AI_Output(hero, self, "Info_Mod_Garond_Mario2_15_02"); //Gerecht?!
	AI_Output(self, hero, "Info_Mod_Garond_Mario2_04_03"); //Hör bloß auf, jetzt den Philosophen raushängen zu lassen.
	AI_Output(self, hero, "Info_Mod_Garond_Mario2_04_04"); //Du kannst von Glück sagen, dass ich eure Betrügerei nicht schon früher geahndet habe.
...
FUNC VOID Info_Mod_Garond_Mario3_B()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Mario3_B_04_01"); //(spöttisch) Ich freue mich darauf.
...
FUNC VOID Info_Mod_Garond_Mario3_A()
{
	...
	AI_Output(self, hero, "Info_Mod_Garond_Mario3_A_04_01"); //Ha, heute scheint mein Glückstag zu sein. Zwei Fliegen mit einer Klappe, richtig?
	AI_Output(self, hero, "Info_Mod_Garond_Mario3_A_04_02"); //Zieh deine Waffe und kämpf um dein Leben!
