...
FUNC VOID Info_Mod_Alissandro_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Hi_23_01"); //Ich bin neuer Erzbaron und Berater von Thorus.
};
...
FUNC VOID Info_Mod_Alissandro_Dieb_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Dieb_23_01"); //Also, normalerweise ist es zwischen 23 und 6 Uhr, wenn er zuschlägt. Um die Zeit halten vier Gardisten Wache, die davon eigentlich etwas mitbekommen müssten:
	AI_Output(self, hero, "Info_Mod_Alissandro_Dieb_23_02"); //Bullit, Cutter, Jackal und Bloodwyn. Dass sie bisher nichts bemerkt haben, macht sie in meinen Augen verdächtig. Zumindest einer wird seine Finger im Spiel haben.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_Unbekannter_23_00"); //Du wurdest doch von Thorus beauftragt, dem Unbekannten den Betrug nachzuweisen.
	AI_Output(hero, self, "Info_Mod_Alissandro_Unbekannter_15_01"); //Ja, und?
	AI_Output(self, hero, "Info_Mod_Alissandro_Unbekannter_23_02"); //Ich weiß, in welcher Hütte der Unbekannte schläft. Wenn du aus der Burg gehst und dann rechts Richtung Arena, dann die Hütte direkt neben Scattys Stand.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_ThorusTot_23_00"); //(benommen) Verdammt, ich habe nichts mehr tun können.
	AI_Output(hero, self, "Info_Mod_Alissandro_ThorusTot_15_01"); //Was ist passiert?
	AI_Output(self, hero, "Info_Mod_Alissandro_ThorusTot_23_02"); //Dieses Drecksschwein Bartholo hat uns zu dieser Höhle geführt, meinte, die Buddler würden sich auflehnen.
	AI_Output(self, hero, "Info_Mod_Alissandro_ThorusTot_23_03"); //Als wir da waren, hat er uns mit einer Horde Gardisten überfallen. Thorus hat er dabei getötet, Arto und ich kämpften bis zum Umfallen, bis wir gemerkt haben, dass wir gegen die Überzahl nicht gewinnen konnten.
	AI_Output(self, hero, "Info_Mod_Alissandro_ThorusTot_23_04"); //Also hat mich Arto mit einem Schlafzauber belegt, damit es aussieht, als wäre ich tot. Er konnte fliehen. Er meinte, wir treffen uns im alten Kastell, oben am Berg.
	AI_Output(self, hero, "Info_Mod_Alissandro_ThorusTot_23_05"); //Er schlug vor, von da aus unseren Gegenschlag zu planen, denn Bartholo hat das Lager übernommen.
	AI_Output(self, hero, "Info_Mod_Alissandro_ThorusTot_23_06"); //Ich werde euch hinführen.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_ZwischenstationA_23_00"); //Hm, der Weg hier ist gut bewacht, wir sollten einen anderen nehmen.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_ZwischenstationB_23_00"); //Von hier aus sollte der Weg nicht mehr allzu schwierig sein.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_Kastell_23_00"); //Da wären wir. Hier werden wir uns auf den Gegenschlag vorbereiten.
	AI_Output(self, hero, "Info_Mod_Alissandro_Kastell_23_01"); //Ich werde mich mit Arto beraten. Komm morgen wieder.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_Botschafter_23_00"); //Es gibt ein Problem und ich will, dass du es löst.
	AI_Output(hero, self, "Info_Mod_Alissandro_Botschafter_15_01"); //Verstehe, was habe ich zu tun?
	AI_Output(self, hero, "Info_Mod_Alissandro_Botschafter_23_02"); //Wir haben Botschafter in alle Lager geschickt, um Hilfe im Kampf gegen Bartholo zu erbitten. Allerdings ist ein Botschafter nicht zurückgekommen.
	AI_Output(hero, self, "Info_Mod_Alissandro_Botschafter_15_03"); //Wo habt ihr ihn hingeschickt?
	AI_Output(self, hero, "Info_Mod_Alissandro_Botschafter_23_04"); //In das Lager der Banditen nahe der Trollschlucht.
	AI_Output(hero, self, "Info_Mod_Alissandro_Botschafter_15_05"); //Und da wundert ihr euch, dass er nicht zurückgekommen ist?
	AI_Output(self, hero, "Info_Mod_Alissandro_Botschafter_23_06"); //Allerdings. Die Banditen würden ihn nicht ohne Vorwarnung umbringen. Wenn sie Interesse daran hätten, ihn zu töten, hätten sie versucht, Lösegeld für ihn zu kassieren.
...
FUNC VOID Info_Mod_Alissandro_BanditenDabei_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_BanditenDabei_23_01"); //Gut, so haben wir einen Verbündeten mehr.
	AI_Output(self, hero, "Info_Mod_Alissandro_BanditenDabei_23_02"); //Hier, nimm deine Belohnung.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_EigentlichBereit_23_00"); //Wir haben nun genug Leute, um das Alte Lager zu stürmen, allerdings gibt es noch ein Problem.
	AI_Output(self, hero, "Info_Mod_Alissandro_EigentlichBereit_23_01"); //Wir kennen die Aufstellung der Gardisten nicht. Und genau da liegt dein nächster Auftrag.
	AI_Output(hero, self, "Info_Mod_Alissandro_EigentlichBereit_15_02"); //Gut, was soll ich machen?
	AI_Output(self, hero, "Info_Mod_Alissandro_EigentlichBereit_23_03"); //Du sollst in das Lager einbrechen und die Karte mit der Aufstellung stehlen.
	AI_Output(hero, self, "Info_Mod_Alissandro_EigentlichBereit_15_04"); //Und wie soll ich in das Lager kommen?
	AI_Output(self, hero, "Info_Mod_Alissandro_EigentlichBereit_23_05"); //Ich habe von den Verwandlungsmagiern einige Spruchrollen bekommen, damit kommst du in das Lager.

	CreateInvItems	(self, ItSc_TrfRabbit, 3);
	B_GiveInvItems	(self, hero, ItSc_TrfRabbit, 3);

	AI_Output(self, hero, "Info_Mod_Alissandro_EigentlichBereit_23_06"); //Du darfst dich nicht zurückverwandeln, bis du in Bartholos Zimmer bist, der Ort, an dem früher Gomez wohnte.
...
FUNC VOID Info_Mod_Alissandro_WarnungLagerEntdeckt_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_WarnungLagerEntdeckt_23_01"); //Verdammt, wir teilen uns auf. Geh zur Goblinhöhle auf dem Weg zum Sumpflager.
	AI_Output(self, hero, "Info_Mod_Alissandro_WarnungLagerEntdeckt_23_02"); //Benutze nicht den Bergpass, ich habe dir den Weg auf der Karte eingezeichnet.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_GotoZufluchten_23_00"); //Es ist nun Zeit, das Lager zurück zu erobern. Ich will, dass du die anderen Zufluchten unserer Leute aufsuchst und sie in das Sektenlager schickst, wo wir eine Beratung abhalten werden.
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_GotoZufluchten_23_04"); //Wenn du bei allen sieben Zufluchten warst, geh ins Sektenlager zum Tempelvorplatz.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_01_23_00"); //Wir haben einen Plan ausgearbeitet, wie wir das Lager einnehmen.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_08_23_00"); //Wir kümmern uns in der Zeit nicht um die Schlacht draußen, sondern suchen Bartholo.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_10_23_00"); //Also, ich fasse es noch mal zusammen:
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_10_23_01"); //1. Schritt: Wir dringen als Sumpfkrauthändler verkleidet in das Lager ein. Als Geleitschutz dienen uns zwei Templer.
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_10_23_02"); //2. Schritt: Makanius gibt für unsere Leute im Lager das Zeichen, die Tore zu öffnen.
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_10_23_03"); //3. Schritt: Die Banditen vom einen und die Garde vom anderen Tor stoßen zu uns und bekämpfen die Gardisten.
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_10_23_04"); //4. Schritt: Unser Auftrag liegt darin, Bartholo zu töten, dabei helfen uns die Heilmagier.
	AI_Output(self, hero, "Info_Mod_Alissandro_Eroberung_10_23_05"); //Gut, dann brechen wir nun auf. Folge mir.
...
FUNC VOID Info_Mod_Alissandro_BartholoTot_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_BartholoTot_23_01"); //Gut, nun ist der Kampf vorbei. In diesem Fall hast du es verdient, belohnt zu werden.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_BelohnungFuerEroberung_23_00"); //Gut, du wurdest nun hierher berufen, um den höchsten Rang im Lager einzunehmen.
	AI_Output(self, hero, "Info_Mod_Alissandro_BelohnungFuerEroberung_23_01"); //Nimm diese Rüstung in Empfang und sprich mir nach:

	CreateInvItems	(hero, Ebr_Armor_H2, 1);

	B_ShowGivenThings	("Schwere Erzbaronrüstung erhalten");

	AI_Output(self, hero, "Info_Mod_Alissandro_BelohnungFuerEroberung_23_02"); //Ich schwöre im Namen jedes Mitgliedes des Lagers, ...
	AI_Output(hero, self, "Info_Mod_Alissandro_BelohnungFuerEroberung_15_03"); //Ich schwöre im Namen jedes Mitgliedes des Lagers, ...
	AI_Output(self, hero, "Info_Mod_Alissandro_BelohnungFuerEroberung_23_04"); //... ob Buddler, Schatten oder Erzbaron, ...
	AI_Output(hero, self, "Info_Mod_Alissandro_BelohnungFuerEroberung_15_05"); //... ob Buddler, Schatten oder Erzbaron, ...
	AI_Output(self, hero, "Info_Mod_Alissandro_BelohnungFuerEroberung_23_06"); //... dass ich das Lager gerecht leite und schütze.
	AI_Output(hero, self, "Info_Mod_Alissandro_BelohnungFuerEroberung_15_07"); //... dass ich das Lager gerecht leite und schütze.
	AI_Output(self, hero, "Info_Mod_Alissandro_BelohnungFuerEroberung_23_08"); //Nun gut, nun bist du Erzbaron des Alten Lagers.
...
FUNC VOID Info_Mod_Alissandro_WasTunAlsErzbaron_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_WasTunAlsErzbaron_23_01"); //Nun, wir kümmern uns um die Probleme des Lagers und versuchen es einflussreicher zu machen.
	AI_Output(hero, self, "Info_Mod_Alissandro_WasTunAlsErzbaron_15_02"); //Und wie soll ich dazu beitragen?
	AI_Output(self, hero, "Info_Mod_Alissandro_WasTunAlsErzbaron_23_03"); //Unser momentanes Ziel ist es, unseren Einfluss auf Khorinis auszuweiten.
	AI_Output(hero, self, "Info_Mod_Alissandro_WasTunAlsErzbaron_15_04"); //Und was kann ich dabei tun?
	AI_Output(self, hero, "Info_Mod_Alissandro_WasTunAlsErzbaron_23_05"); //Geh in die Stadt und rede mit Larius, dem Statthalter.
	AI_Output(hero, self, "Info_Mod_Alissandro_WasTunAlsErzbaron_15_06"); //Larius' Posten haben doch die Paladine übernommen.
	AI_Output(self, hero, "Info_Mod_Alissandro_WasTunAlsErzbaron_23_07"); //Verdammt, das wusste ich nicht.
	AI_Output(hero, self, "Info_Mod_Alissandro_WasTunAlsErzbaron_15_08"); //Und was soll ich jetzt machen?
	AI_Output(self, hero, "Info_Mod_Alissandro_WasTunAlsErzbaron_23_09"); //Der Schatten Whistler ist bereits auf dem Weg in die Stadt. Unterhalte dich mit ihm.
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_Daemonenritter_23_00"); //Wer sind diese Leute? Was wollen sie von uns?
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Daemonenritter_23_02"); //Gut, folgt mir!
...
{
	AI_Output(self, hero, "Info_Mod_Alissandro_AtCavalorn_23_00"); //Hier dürften wir in Sicherheit sein. Und nun erklär mir, was die Leute in den schwarzen Rüstungen von uns wollen.
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_AtCavalorn_23_03"); //Nun gut, jetzt sollten wir den nächsten Schritt planen.
	AI_Output(self, hero, "Info_Mod_Alissandro_AtCavalorn_23_04"); //Ich werde etwas darüber nachdenken, wir unterhalten uns morgen darüber.
...
FUNC VOID Info_Mod_Alissandro_Flucht_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_01"); //Wir sind hier nicht lange sicher, vermutlich sucht man schon nach uns.
	AI_Output(hero, self, "Info_Mod_Alissandro_Flucht_15_02"); //Dann treten wir denen, die uns suchen, in den Arsch.
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_03"); //Dafür sind es zu viele und wir können keine Verstärkung anfordern. Wir müssen erst einmal fliehen und Unterstützung anfordern und da hätte ich einen Plan ...
	AI_Output(hero, self, "Info_Mod_Alissandro_Flucht_15_04"); //Wie sieht der aus?
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_05"); //Die Paladine sind mit Gomez' Leuten verfeindet und sie sind eine starke Gilde, also können wir vielleicht auf ihre Unterstützung hoffen.
	AI_Output(hero, self, "Info_Mod_Alissandro_Flucht_15_06"); //Warum sollten die Paladine ausgerechnet uns helfen?
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_07"); //(belustigt) Oh, ich habe einen bemerkenswert guten Draht zu ihnen ...
	AI_Output(hero, self, "Info_Mod_Alissandro_Flucht_15_08"); //Wie das?
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_09"); //Na gut, ich denke, ich sollte es dir erzählen ... Bevor ich mich auf Khorinis als Händler niederließ, war ich Paladin in Myrtana und mit Hagen befreundet.
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_10"); //Wir schlugen zusammen die Schlacht von Ishtar, als es von uns besetzt war und der Assassinenführer Zuben die Stadt zurückerobern wollte ... Nun, wir haben den Angriff damals abgewehrt, heute haben die Assassinen allerdings wieder die Macht in Varant...
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_11"); //Das tut allerdings nichts zur Sache. Ich bin jedenfalls mit Lord Hagen befreundet und ich denke, er wird uns helfen.
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_13"); //Das ist wahr und macht es schwieriger. Nur haben die Wachen beim Austauschplatz dieses Zeug für sich entdeckt, das sie in Khorata verkaufen ... Wie hieß es noch mal? Frustspender?
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Flucht_23_15"); //Gut, viel Glück!
...
FUNC VOID Info_Mod_Alissandro_Hagen_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Hagen_23_01"); //Sehr gut. Nun warten wir, bis seine Leute kommen.
...
FUNC VOID Info_Mod_Alissandro_OricDa_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_OricDa_23_01"); //Das ist nur die Truppe, mit der wir die ersten Schritte ausführen werden. Für die Befreiung der Mine werden mehr zu uns stoßen und für die Belagerung ebenfalls.
	AI_Output(self, hero, "Info_Mod_Alissandro_OricDa_23_02"); //Mit Orics Truppe nehmen wir die Wege aus dem Minental ein. Wir werden nun gleich losgehen und die Alte Mine wie auch den Austauschplatz erobern.
	AI_Output(hero, self, "Info_Mod_Alissandro_OricDa_15_03"); //Was machen wir mit den Beliarschreinen?
	AI_Output(self, hero, "Info_Mod_Alissandro_OricDa_23_04"); //Die Orks in der Orkstadt sind mit allen Menschen verfeindet. Die in der Festung unterhalten Beziehungen mit dem Schwarzmagier, der die Dämonenritter und die Beschwörer anführt ...
	AI_Output(self, hero, "Info_Mod_Alissandro_OricDa_23_05"); //... dieser unterstützt Gomez' Pläne allerdings nicht und hat die Altäre für die Ritter, die mit Gomez zusammenarbeiten, gesperrt.
	AI_Output(hero, self, "Info_Mod_Alissandro_OricDa_15_06"); //Das macht die Sache etwas einfacher.
	AI_Output(self, hero, "Info_Mod_Alissandro_OricDa_23_07"); //Gut. Wir ziehen's jetzt durch. Folgen wir Oric.
...
FUNC VOID Info_Mod_Alissandro_Lockvogel_Info()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_01"); //Du hast noch die Teleportrune in das Lager ...
	}
	else
	{
		AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_02"); //Hier hast du eine Teleportrune ins Lager ...
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_03"); //Sie bringt dich in das Haus der Heilmagier. Sie sind im Haus gefangen, es wird bewacht.
	AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_04"); //Allerdings stehen sie auf unserer Seite. Sie werden dich also nicht verraten, wenn du dich in ihr Gebäude teleportierst.
	AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_05"); //Zusammen mit den Heilmagiern tötest du die Wache des Hauses und nimmst ihre Rüstung.
	AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_06"); //Gehe mit dieser zu Gomez und versuche ihn und seine Leute aus dem Lager in den Wald in der Nähe der Hütte zu locken.
	AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_07"); //Dort werden wir einen Hinterhalt vorbereiten.
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Lockvogel_23_09"); //Überlass das Makanius. Er scheint einen Plan zu haben. Nun geh und erfülle deine Aufgabe ... viel Glück.
...
FUNC VOID Info_Mod_Alissandro_Erzbaron_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alissandro_Erzbaron_23_01"); //Ich war vorher Kaufmann in Khorinis und habe Erz an- und verkauft. Also bin ich nach dem Fall der Barriere in das Minental gekommen, um zu sehen, wie es um das Erz steht.
	AI_Output(self, hero, "Info_Mod_Alissandro_Erzbaron_23_02"); //Mein Erkundigungen führten mich ins Alte Lager.
	AI_Output(self, hero, "Info_Mod_Alissandro_Erzbaron_23_03"); //Als ich mich ein Weilchen mit Thorus unterhalten hatte, fragte er mich, ob ich ihm nicht mit meinem Handelsgeschick unter die Arme greifen könnte.
	AI_Output(self, hero, "Info_Mod_Alissandro_Erzbaron_23_04"); //Nun ja, da ich ja auch das Erz in Aussicht hatte, habe ich natürlich zugestimmt.
	AI_Output(self, hero, "Info_Mod_Alissandro_Erzbaron_23_05"); //Und so arbeiten wir hier nun zusammen und ich bin sein oberster Berater.
};
