...
FUNC VOID Info_Mod_Bibliothekar_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Hi_11_01"); //(abwesend) Die Geschichte des Hofstaats Teil 1, Die Geschichte des Hofstaats Teil 2, Die Geschichte des Hofstaats Teil 3, Die Geschichte ...
	AI_Output(hero, self, "Info_Mod_Bibliothekar_Hi_15_02"); //Ich sehe schon, spannende Bettlektüre.
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Hi_11_03"); //Ja, ja. (murmelt) 101 Kräuterrezepte für die betagte Tagesmutter, wer hatte das gleich noch ausgeliehen? Richtig, Regina.
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Hi_11_04"); //(laut) Kann ich sonst noch etwas für Sie tun?
};
...
FUNC VOID Info_Mod_Bibliothekar_Ausleihen_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Ausleihen_11_01"); //(scharf) Finger weg!
};
...
FUNC VOID Info_Mod_Bibliothekar_Buerger_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Buerger_11_01"); //(genervt) Ja, wirklich sehr schönes Wetter draußen ...
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Buerger_11_02"); //(verdutzt) Wie? Jemand fragt mich nach einem Buch?
	AI_Output(hero, self, "Info_Mod_Bibliothekar_Buerger_15_03"); //Allerdings.
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Buerger_11_04"); //Da muss ich doch gleich mal nachdenken.
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Buerger_11_05"); //(freundlich) Damit solltest du was anfangen können. Ich habe es doppelt, also behalte es ruhig.

	B_GiveInvItems	(self, hero, ItWr_KhorataGeschichte, 1);

	AI_Output(self, hero, "Info_Mod_Bibliothekar_Buerger_11_06"); //Und wenn du dich für mehr interessierst, komm nur vorbei, es ist alles da!
...
{
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Glorie_11_00"); //Wo habe ich bloß das verdammte Ding gelassen ...
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Glorie_11_01"); //(hält inne) Ihr seid es? Ohh, verzeiht vielmals, ich eile.
	...
	AI_Output(self, hero, "Info_Mod_Bibliothekar_Glorie_11_02"); //Hier, bitte sehr.
