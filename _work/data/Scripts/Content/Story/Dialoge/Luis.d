...
FUNC VOID Info_Mod_Luis_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_01"); //Das würde wohl jeden Jäger belasten!
	...
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_03"); //Du meinst so wie ein Dutzend anderer Jäger?
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_04"); //Der Konkurrenzkampf vor der Stadt ist nicht mehr auszuhalten und viel bekommt man auch nicht für ein einfaches Fell.
	AI_Output(hero, self, "Info_Mod_Luis_Hi_15_05"); //Das Eisgebiet ist doch recht groß, irgendwo muss da doch was zu holen sein?
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_06"); //Mein Kumpel Randi hat was ähnliches gesagt, bevor er aufgebrochen ist, und bis heute habe ich nichts mehr von ihm gehört.
	AI_Output(hero, self, "Info_Mod_Luis_Hi_15_07"); //Was hatte er denn vor?
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_08"); //Er wollte den alten Turm am Eissee als ein Lager für die Nacht nutzen.
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_09"); //Eigentlich keine schlechte Idee, nur wer weiß, was sich nach all den Jahren dort eingenistet hat.
	...
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_12"); //Wenn du unbedingt auch dort sterben willst, bitte!
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_13"); //Aber du solltest vorher noch bei Keith vorbei schauen. Randi wollte ihn von seiner Idee überzeugen.
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_14"); //Vielleicht hast du ja Glück und sie sind noch nicht aufgebrochen.
	AI_Output(self, hero, "Info_Mod_Luis_Hi_28_15"); //Hier hast du eine Karte, mit ihr solltest du zu seiner Hütte im Wald gelangen, Viel Erfolg.
...
FUNC VOID Info_Mod_Luis_Vigonza_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_01"); //Ich hab es ja befürchtet, allerdings hab ich gehofft, dass ich falsch liege.
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_02"); //Jetzt, wo ich die Gewissheit von seinem Tod habe, mache ich mir noch mehr sorgen um Pilar.
	AI_Output(hero, self, "Info_Mod_Luis_Vigonza_15_03"); //Jetzt, wo du es sagst: Wo ist er eigentlich?
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_04"); //Er ist vor kurzem abgehauen, meinte, er hält es hier nicht mehr aus.
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_05"); //Er wollte zu seinem Bruder, er wohnt in der Nähe des Goblinfriedhofes, beim zerfallenen Wachturm.
	...
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_07"); //Bei den normalen Goblins trifft das auch zu, allerding gab es vor ein paar Jahren einen Dämonenbeschwörer unter ihnen.
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_08"); //Der hat anscheinend den anderen beigebracht einen Friedhof anzulegen.
	AI_Output(hero, self, "Info_Mod_Luis_Vigonza_15_09"); //Das klingt für mich aber nicht sehr gut.
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_10"); //Keine Panik, die Goblins haben wir vor langer Zeit in einer Schlacht an einem der Wachtürme besiegt.
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_11"); //Als die Goblins versucht haben ihn einzunehmen haben wir fast den ganzen Klan getötet. Und die paar, die überlebt haben, wollten nur ihre Toten beerdigen.
	AI_Output(hero, self, "Info_Mod_Luis_Vigonza_15_12"); //Und der Dämonenbeschwörer?
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_13"); //Ist auch tot. Er hat sogar den größten Grabstein auf dem Friedhof.
	AI_Output(self, hero, "Info_Mod_Luis_Vigonza_28_14"); //Wenn du willst zeig ich dir den Friedhof. Ich will sowieso zu Pilar und das liegt auf dem Weg.
...
{
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof_28_00"); //So, da wären wir. Ist heute ja nicht viel los hier.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof_28_01"); //Komm mit, ich zeig dir das Grab des Dämonenbeschwörers.
...
{
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof2_28_00"); //Hier ist es. Schon irgendwie eigenartig, wie ein schlauer Goblin die Goblin-Masse zu so etwas wie dem Bau dieses Friedhofs bewegen kann.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof2_28_01"); //Was war das ... hast du das auch gehört?!
...
{
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof3_28_00"); //Verdammt, die haben mich gerade aber erschreckt. Ich hab die wirklich nicht kommen gehört.
	...
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof3_28_02"); //Ach und wenn schon. Hier liegen doch sowieso nur Leichen.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof3_28_03"); //Die sollten sich besser um ihre Rübenfelder kümmern, als hier den ganzen Tag rumzulungern.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof3_28_04"); //Lass uns jetzt zur Hütte von Pilars Bruder gehen.
...
{
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof4_28_00"); //Lass mich raten, wir kommen gerade zu spät, oder?
	...
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof4_28_02"); //Mhm ... das könnte heißen, das er vielleicht doch nicht tot ist.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof4_28_03"); //Wir sollten mal den zerfallenen Turm unter die Lupe nehmen. Vielleicht gibt es ja da einen Hinweis auf seinen verbleib.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof4_28_04"); //Ich lass die Wachen hier Posten beziehen, falls doch noch ein paar von denen aus der Erde kommen.
...
{
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof5_28_00"); //Wollen wir hoffen, dass es von dem wirklich nur einen gab.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof5_28_01"); //Wenn die alle so stark wären, hätten wir ein ganz schönes Problem.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof5_28_02"); //Ich denke für heute haben die Goblins erst mal genug. Ich geh jetzt zurück in die Taverne.
	AI_Output(self, hero, "Info_Mod_Luis_AtFriedhof5_28_03"); //Ach ja, du sollst dich auch bei Thys melden. Ich glaube er will sich bedanken.
