...
FUNC VOID Info_Mod_Gravo_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_04_01"); //(eifrig) Das kann ich nicht sagen. Das heißt, ich würde ja gern, aber dann würdest du sicher versuchen, mich von hier zu vertreiben.
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_04_02"); //War das jetzt schon zu viel verraten?
...
FUNC VOID Info_Mod_Gravo_Hi_C()
{
	...
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_C_04_01"); //(durchgeknallt) Am liebsten würde ich mit der Spitzhacke nur noch irgendwelchen Typen die Birne einschlagen.
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_C_04_02"); //Ach ... so was sagt man nicht, oder?
};

FUNC VOID Info_Mod_Gravo_Hi_B()
{
	AI_Output(hero, self, "Info_Mod_Gravo_Hi_B_15_00"); //Du bist ... auf Schatzsuche?
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_B_04_01"); //Psst! Hat dir Kyle auch davon erzählt?
	AI_Output(hero, self, "Info_Mod_Gravo_Hi_B_15_02"); //Klar.
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_B_04_03"); //(beleidigt) Er hat mir versprochen, es bliebe unser Geheimnis.
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_B_04_04"); //Na ja, aber wenn du davon weißt, bist du doch jetzt auch sicher hinter Garaz' und Pachos' Schatz her.
	AI_Output(hero, self, "Info_Mod_Gravo_Hi_B_15_05"); //Ähm ... ja.
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_B_04_06"); //Dann wären wir ja so was wie Konkurrenten.
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_B_04_07"); //Hmm ... sollte ich dich dann nicht besser sofort töten?
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_B_04_08"); //Nein, warte, immer zuerst reden, haben sie gesagt. Ich soll meine gute Ausbildung nicht vergessen.
...
FUNC VOID Info_Mod_Gravo_Hi_A()
{
	...
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_A_04_01"); //Ja! Genau! Das Gefühl habe ich die ganze Zeit! Irgendetwas muss ich verloren haben.
	AI_Output(self, hero, "Info_Mod_Gravo_Hi_A_04_02"); //Ich komm nur nicht drauf, was es war ...
};
...
FUNC VOID Info_Mod_Gravo_WasFuerSchatz_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Gravo_WasFuerSchatz_04_01"); //(hämisch) Das hat dir Kyle also nicht erzählt, wie?
	AI_Output(self, hero, "Info_Mod_Gravo_WasFuerSchatz_04_02"); //Dass Garaz und Pacho hier ihren ganzen Reichtum vergraben haben sollen, bevor sie dem Minental den Rücken gekehrt haben, um eine Musiktruppe zu gründen?
	AI_Output(self, hero, "Info_Mod_Gravo_WasFuerSchatz_04_03"); //Dann werde ich es dir ganz bestimmt auch nicht sagen!
	AI_Output(hero, self, "Info_Mod_Gravo_WasFuerSchatz_15_04"); //Eine Musiktruppe?
	AI_Output(self, hero, "Info_Mod_Gravo_WasFuerSchatz_04_05"); //Das war schon immer ihr großer Traum, haben sie gesagt.
	AI_Output(self, hero, "Info_Mod_Gravo_WasFuerSchatz_04_06"); //Einen Künstlernamen hatten sie auch schon, eine Kombination aus ihren Namen. Gazpacho oder so.
	AI_Output(self, hero, "Info_Mod_Gravo_WasFuerSchatz_04_07"); //Als Snaf das gehört hat, hat er sich gar nicht mehr eingekriegt. Frag mich nicht, warum.
};
...
FUNC VOID Info_Mod_Gravo_IchHacken_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Gravo_IchHacken_04_01"); //Nichts da! Das hier ist mein Revier.
	AI_Output(self, hero, "Info_Mod_Gravo_IchHacken_04_02"); //Sobald ich den Schatz gehoben habe und über alle Berge davon bin und mir von einer südländischen Schönheit Rum in den Mund träufeln lasse, kannst du's ja noch mal versuchen!
};
...
FUNC VOID Info_Mod_Gravo_Schatz_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Gravo_Schatz_04_01"); //Schlechter Versuch. So einfach kriegst du mich hier nicht weg.
	...
	AI_Output(self, hero, "Info_Mod_Gravo_Schatz_04_04"); //(enttäuscht) Tatsächlich? Und ich dachte noch, sein Pisspott in der Tür wäre bloß Zufall.
	AI_Output(self, hero, "Info_Mod_Gravo_Schatz_04_05"); //(in Rage) Am liebsten würde ich seine feuchte Scheiße nehmen und sie ihm...
	AI_Output(self, hero, "Info_Mod_Gravo_Schatz_04_06"); //(entspannter) ... zur Abfallgrube tragen.
	AI_Output(self, hero, "Info_Mod_Gravo_Schatz_04_07"); //Was soll's, finde ich halt anderswo meinen Schatz. Irgendwo wird schon einer sein.
...
{
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_00"); //(tadelnd) Na, na, na, da hat doch nicht etwa jemand meine Abwesenheit ausgenutzt?
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_01"); //Du dachtest du wohl, du könntest mich so einfach abschütteln?
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_02"); //(nachäffend) Kyle hat dich nur angeschmiert, ahahaha.
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_03"); //Eigentlich sollte ich dich für diesen Verrat - he, was hast du denn da gefunden?!
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_04"); //Warte mal - das sind doch die alten Instrumente von Garaz und Pacho! Die sie nicht mehr mitnehmen konnten! Das ist also ihr Schatz!

	AI_PlayAni	(self, "T_PLUNDER");

	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_05"); //(euphorisch) Damit kann ich endlich meine eigene Gruppe gründen! Den Jungs mal ein bisschen mit Musik anheizen.
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_06"); //Ähm, du wirst mir doch helfen, Mitglieder zu finden, oder? Die meisten hier sprechen nämlich nicht mit mir.
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_04_07"); //Vielleicht könntest du ein paar Buddler überzeugen. Die haben ja eh nichts zu tun.
...
FUNC VOID Info_Mod_Gravo_Lagermusik_B()
{
	...
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik_B_04_01"); //(trotzig) Ja, klar. Ich bin nicht auf deine Hilfe angewiesen.
...
{
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik2_04_00"); //Sieh an, mit deiner Hilfe haben wir es wirklich geschafft!

	if (Npc_KnowsInfo(hero, Info_Mod_Seraphia_Lagermusik))
	{
		AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik2_04_01"); //Wir haben schon unsere Aufgaben besprochen, Graham wird die Gitarre spielen, Mud die Trommel, Guy die Rassel, Seraphia tanzen, und ich werde singen.

		B_GivePlayerXP	(100);
	}
	else
	{
		AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik2_04_02"); //Wir haben schon unsere Aufgaben besprochen, Graham wird die Gitarre spielen, Mud die Trommel,  Guy die Rassel, und ich werde singen.

		B_GivePlayerXP	(50);
	};

	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik2_04_03"); // Jetzt brauchen wir nur noch einen guten Namen. Uns fällt da nichts ein...
...
{
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik2_D_04_00"); //Meinetwegen. Der Name ist ja eh nicht so wichtig.
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik2_D_04_01"); //Wir proben jetzt einen Tag und treten dann morgen Abend um 19 Uhr erstmals auf der Bühne auf.
	AI_Output(self, hero, "Info_Mod_Gravo_Lagermusik2_D_04_02"); //Bis dahin muss ich mir noch einen Liedtext ausdenken...
